...
```{r}
all_nodes <- mc3_nodes_cleaned3 %>%
  mutate(type2 = ifelse(is.na(category), type, ifelse(category == 'seafood' & type == 'Company', 'Seafood Company', type))) %>%
  mutate(type3 = ifelse(type2=='Company','Other Company',type2)) %>%
  select(-type2) %>%
  rename(type2=type3)

write_csv(all_nodes,'C:\\Users\\kkb74\\downloads\\all_nodes.csv')
